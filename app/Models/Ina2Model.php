...
class Ina2Model extends Model
{
    ...
    public function getStatisticIna2()
    {
        $db = \Config\Database::connect();
        //$query = $db->query("select TIME(created_at) as time, battery, id from battery");
        $query = $db->query("SELECT * FROM (SELECT TIME(created_at) as time, data_volt, data_arus, id FROM sensor_ina2 ORDER BY id DESC LIMIT 6) sub ORDER BY id ASC");
